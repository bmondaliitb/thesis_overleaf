...
Jet reconstruction is described in~\cref{sec:physics_object_reconstruction}. The uncertainties comes mainly from the jet energy scale and resolution, jet vertex tagging. Jet energy scale which corrects the jet energy with particle level energy.
Jets are calibrated with sequence of simulation-based corrections then several \emph{in-situ} techniques are used to correct the differences between data and simulation. 
